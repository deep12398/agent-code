...
> 覆盖范围：第 02-48 篇全部配套代码。第 19-48 篇按连续项目组织（采购 Agent、
> LLMOps 平台），不依赖外部 API Key，全部离线可运行、有测试覆盖。第 49-50 篇为
> 面试番外，文章明确无新代码。
...
| `series_projects/chapter_runner.py` | 统一章节入口（支持 11-48） | 我新写 |
| `agent_examples/*.py` | 离线可运行的轻量基础设施 | 我新写 |
| `11_*` 到 `48_*` 的 `demo.py` | 各文章章节入口 | 我新写；调用 `series_projects.chapter_runner` |
| `tests/test_all_demos.py` | 全量运行验证（11-48 每章跑一遍） | 我新写 |
...
| 第 49-50 篇 面试番外 | （无新代码） | 文章建议对照重跑 02-48 篇的配套代码 |
